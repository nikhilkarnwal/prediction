...
net_t = MTP(backbone, num_modes=2, input_shape=(3,200,200)).to(device)
loss = MTPLoss(2)
...
for epoch in range(start_epoch, start_epoch + num_epochs):
    ...
    for i, data in enumerate(tr_dl):
        ...
        fut = torch.unsqueeze(fut, dim=1)

        # Forward pass:
        op_traj = net_t(img, hist[:,-1,-3:])
        ...
        if i % iters_log == iters_log - 1:
            ...
            tb_fig_train = tb_traj_pt_plots(img_vis[0:8],
    ...
    with torch.no_grad():
        ...
        for k, data_val in enumerate(val_dl):
            ...
            waypts_e = waypts_e.permute(1, 0, 2).float().to(device)
            grid_idcs = grid_idcs.float().to(device)
            fut = torch.squeeze(fut, dim=1)
            
            # Log images from first val batch into tensorboard
            if k == 0:
                tb_fig_val = tb_traj_pt_plots(img_vis[0:8],
                                                hist[:, 0:8, :].permute(1, 0, 2).detach().cpu(),
                                                op_traj[0:8,:24].reshape((8,12,2)).detach().cpu(),
                                                fut[0:8].detach().cpu(),
                                                svf_e[0:8],
                                                extent=config['grid_extent'])
